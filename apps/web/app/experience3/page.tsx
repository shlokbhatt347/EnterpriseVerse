...
function CauseMap({ state, kpis }: { state: SimulationState; kpis: ReturnType<typeof calculateKpis> }) { return <section className="panel big-panel"><span className="micro">CAUSALITY ENGINE · LIVE STATE</span><h2>Why is the enterprise performing this way?</h2><p className="lead">Trace the current business position from financial state to operating drivers.</p><div className="cause-map"><Node title="REVENUE" value={money(state.business.revenue)} /><em>minus</em><Node title="EXPENSES" value={money(state.business.expenses)} /><em>creates</em><Node title="PROFIT" value={money(kpis.profit)} /><em>defines</em><Node title="MARGIN" value={`${kpis.grossMargin}%`} /></div><div className="cause-explain"><strong>Primary operating signals</strong><span>Cash runway: {kpis.cashRunwayDays} days</span><span>Customers: {state.business.customers.length}</span><span>Market share: {state.business.marketShare.toFixed(1)}%</span><span>Reputation: {state.business.reputation}/100</span></div></section>; }
