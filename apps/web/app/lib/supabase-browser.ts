...
function readSession(): Session | null {
  try { const raw = window.localStorage.getItem(STORAGE_KEY); return raw ? JSON.parse(raw) as Session : null; } catch { return null; }
}

function writeSession(session: Session | null) {
  try { if (session) window.localStorage.setItem(STORAGE_KEY, JSON.stringify(session)); else window.localStorage.removeItem(STORAGE_KEY); } catch { /* storage may be unavailable */ }
}

function userFromSession(session: Session): BrowserUser {
  return { id: session.user.id, email: session.user.email ?? null, displayName: typeof session.user.user_metadata?.display_name === "string" ? session.user.user_metadata.display_name : "Founder", emailConfirmed: Boolean(session.user.email_confirmed_at) };
}
...
async function refreshIfNeeded(): Promise<Session | null> {
  ...
  try {
    const next = await request<Session>("/auth/v1/token?grant_type=refresh_token", { method: "POST", body: JSON.stringify({ refresh_token: session.refresh_token }) });
...
export async function restoreSessionFromUrl() {
  ...
  if (accessToken && refreshToken) {
    ...
    const session: Session = { access_token: accessToken, refresh_token: refreshToken, expires_in: Number.isFinite(expiresIn) ? expiresIn : 3600, expires_at: Math.floor(Date.now() / 1000) + (Number.isFinite(expiresIn) ? expiresIn : 3600), user };
...
export async function getCurrentUser(): Promise<BrowserUser | null> {
  ...
  if (!session) return null;
  try { const user = await request<Session["user"]>("/auth/v1/user", { method: "GET" }, session.access_token); return userFromSession({ ...session, user }); }
  catch { writeSession(null); return null; }
}
...
export async function signInWithEmail(email: string, password: string) {
  ...
  writeSession(session); return userFromSession(session);
}

export async function signUpWithEmail(email: string, password: string, displayName: string) {
  const redirectTo = `${window.location.origin}${window.location.pathname.replace(/\/auth\/signup\/?$/, "/")}`;
  const session = await request<Session>(`/auth/v1/signup?redirect_to=${encodeURIComponent(redirectTo)}`, { method: "POST", body: JSON.stringify({ email, password, data: { display_name: displayName } }) });
  if (session?.access_token) writeSession(session);
  return { user: session?.user ? userFromSession(session) : null, requiresVerification: !session?.access_token };
}

export async function requestPasswordReset(email: string) {
  const redirectTo = `${window.location.origin}${window.location.pathname.replace(/\/auth\/recover\/?$/, "/auth/reset")}`;
...
export async function signOut() {
  const session = await refreshIfNeeded();
  if (session) { try { await request<unknown>("/auth/v1/logout", { method: "POST" }, session.access_token); } catch { /* local session still gets cleared */ } }
  writeSession(null);
}
...
async function authenticatedRequest<T>(path: string, init: RequestInit = {}) {
  ...
  return request<T>(path, init, session.access_token);
}
...
export async function saveCloudSave(key: string, payload: unknown) {
  await authenticatedRequest("/rest/v1/business_saves?on_conflict=user_id,save_key", { method: "POST", headers: { Prefer: "resolution=merge-duplicates,return=minimal" }, body: JSON.stringify({ save_key: key, payload, updated_at: new Date().toISOString() }) });
}

export async function deleteCloudSave(key: string) {
  await authenticatedRequest(`/rest/v1/business_saves?save_key=eq.${encodeURIComponent(key)}`, { method: "DELETE" });
}
