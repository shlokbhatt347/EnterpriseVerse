...
const PRODUCTION_SITE_URL =
  "https://shlokbhatt347.github.io/EnterpriseVerse/";
...
export async function signUpWithEmail(
   ...
   * /auth/verified
   *
   * NEXT_PUBLIC_SITE_URL takes priority when configured.
   * Otherwise production builds use the GitHub Pages URL.
   */
...
export async function loadCloudSave<T>(
  key: string,
): Promise<T | null> {
  const rows =
    await authenticatedRequest<SaveRow[]>(
      `/rest/v1/business_saves?select=save_key,payload,updated_at&save_key=eq.${encodeURIComponent(
        key,
      )}&limit=1`,
    );

  return (
    (rows[0]?.payload as T | null) ??
    null
  );
}
...
export async function deleteCloudSave(
  ...
  await authenticatedRequest(
    `/rest/v1/business_saves?save_key=eq.${encodeURIComponent(
      key,
    )}`,
    {
      method: "DELETE",
...
export async function loadProfile() {
  const rows =
    ...
    >(
      "/rest/v1/profiles?select=user_id,display_name,avatar_url&limit=1",
    );
