...
export async function getCurrentUser(): Promise<BrowserUser | null> {
  ...
  if (!session) return null;
  try {
    const user = await request<Session["user"]>("/auth/v1/user", { method: "GET" }, session.access_token);
...
export async function saveCloudSave(key: string, payload: unknown) {
  const session = await refreshIfNeeded();
  if (!session) throw new Error("Authentication required.");
  await request("/rest/v1/business_saves?on_conflict=user_id,save_key", {
    method: "POST",
    headers: { Prefer: "resolution=merge-duplicates,return=minimal" },
    body: JSON.stringify({ user_id: session.user.id, save_key: key, payload, updated_at: new Date().toISOString() }),
...
export async function updateProfile(displayName: string, avatarUrl: string | null = null) {
  const session = await refreshIfNeeded();
  if (!session) throw new Error("Authentication required.");
  const rows = await request<Array<{ user_id: string; display_name: string; avatar_url: string | null }>>("/rest/v1/profiles?on_conflict=user_id", {
    method: "POST",
    headers: { Prefer: "resolution=merge-duplicates,return=representation" },
    body: JSON.stringify({ user_id: session.user.id, display_name: displayName, avatar_url: avatarUrl, updated_at: new Date().toISOString() }),
  }, session.access_token);
  return rows[0] ?? null;
}
