...
async function refreshIfNeeded(): Promise<Session | null> {
  ...
  const expiresAt =
    current.expires_at ??
    Math.floor(Date.now() / 1000) + current.expires_in;
...
export async function loadCloudSave<T>(key: string): Promise<T | null> {
  const rows = await authenticatedRequest<SaveRow[]>(
    `/rest/v1/business_saves?select=save_key,payload,updated_at&save_key=eq.${encodeURIComponent(key)}&limit=1`,
  );
...
export async function deleteCloudSave(key: string) {
  await authenticatedRequest(
    `/rest/v1/business_saves?save_key=eq.${encodeURIComponent(key)}`,
    { method: "DELETE" },
  );
}
