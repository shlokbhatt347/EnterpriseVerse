...
import { DelayedConsequences } from "../experience3/delayed-consequences";
import "./play.css";
...
export default function PlayPage() {
  ...
  useEffect(() => {
    try { const raw = window.localStorage.getItem(SAVE_KEY); if (raw) setState(JSON.parse(raw) as SimulationState); }
  ...
  const decisionOptions = useMemo<DecisionOption[]>(() => event?.choices.map((choice) => ({
    id: choice.id, title: choice.label,
  ...
  const commit = (choice: SimulationChoice) => {
    setSelected(choice.id); setDecisionBaseline(state); setCommittedChoice(choice);
    setMessage("Decision locked. Its immediate and delayed consequences are now being remembered.");
    setState((current) => current ? applyChoice(current, choice) : current);
  };
  const endDay = () => { setSelected(null); setCommittedChoice(null); setDecisionBaseline(null); setMessage("Day advanced. Customers, competitors, the market and scheduled consequences have reacted."); setState((current) => current ? advanceDay(current) : current); };

  return <main className="play-shell">
    <header className="play-topbar"><a href="/EnterpriseVerse/" className="play-brand">ENTERPRISEVERSE</a><div className="play-day"><span className="live-dot" aria-hidden="true" /> DAY {state.business.day}<span>·</span>{state.business.name}</div><div className="play-cash">{money(state.business.cash)}</div></header>
    <nav className="p27-mobile-tabs" aria-label="Founder navigation">{["overview", "world", "founder"].map((item) => <button key={item} type="button" className={tab === item ? "active" : ""} onClick={() => setTab(item as Tab)}>{item === "overview" ? "⌂ Command" : item === "world" ? "◈ World" : "◉ Founder"}</button>)}</nav>
    <div className="play-grid">
      ...
      <section className={`play-main ${tab === "world" ? "p27-world-focus" : ""}`}>
        ...
        <DelayedConsequences state={state} />
