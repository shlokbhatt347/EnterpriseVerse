...
import { buildPhase4DecisionBriefs, buildPhase4IntelligenceReport, buildPhase4StrategicPlan, buildPhase4Timeline, phase4MetricBand } from "@enterpriseverse/simulation";
...
export default function Phase4Page() {
  const { state, status, error, endDay, commitChoice } = useSimulation();
  ...
  const choices = useMemo(() => state ? state.events.flatMap((event) => event.choices).slice(0, 8) : [], [state]);
  const decisionBriefs = useMemo(() => state ? buildPhase4DecisionBriefs(state, choices) : [], [state, choices]);
  ...
  const choose = async (choice: SimulationChoice) => { await commitChoice(choice); };

  return (
    <main className="phase4-page">
      <header className="phase4-header"><div><span className="phase4-kicker">PHASE 4 · COMMAND CENTRE</span><h1>{state.business.name}</h1><p>Day {state.business.day} · {report.market.trend} market · {report.market.confidence.value}/100 confidence</p></div><button className="phase4-advance" onClick={() => void endDay()} disabled={status === "saving"}>{status === "saving" ? "Syncing…" : "Advance day"}</button></header>
      <section className="phase4-grid phase4-metrics" aria-label="Executive metrics">{cards.map(([label, value, band]) => <article className={`phase4-card phase4-${band}`} key={label}><span>{label}</span><strong>{value}</strong></article>)}</section>
      <section className="phase4-grid phase4-main">
        <article className="phase4-panel phase4-brief"><div className="phase4-panel-head"><div><span className="phase4-kicker">EXECUTIVE BRIEFING</span><h2>{report.signals[0]?.title ?? "Business is operating normally"}</h2></div><span className="phase4-pill">{plan.priority.replace("_", " ")}</span></div><p>Demand is <b>{report.market.trend}</b>, competitive pressure is <b>{Math.round(report.market.competitivePressure.value)}/100</b>, and customer satisfaction is <b>{Math.round(report.customers.satisfaction.value)}/100</b>.</p><div className="phase4-signals">{report.signals.slice(0, 5).map((signal) => <div className="phase4-signal" key={signal.id}><b>{signal.title}</b><span>{signal.detail}</span><small>{signal.confidence} · {signal.source}</small></div>)}</div></article>
      ...
      <section className="phase4-grid phase4-main">
        <article className="phase4-panel"><div className="phase4-panel-head"><div><span className="phase4-kicker">DECISION CENTRE</span><h2>Available decisions</h2></div><span className="phase4-pill">validated by authority</span></div>{decisionBriefs.length === 0 ? <p>No decision choices are currently pending.</p> : <div className="phase4-decisions">{decisionBriefs.map((brief) => { const choice = choices.find((item) => item.id === brief.choiceId); return <div className="phase4-decision" key={brief.choiceId}><div><b>{brief.label}</b><span>{brief.horizon.replace("_", " ")} · affects {brief.affectedSystems.join(", ")}</span><small>{brief.uncertainty}</small></div><div className="phase4-deltas"><span>Cash {brief.preview.projected.cashDelta >= 0 ? "+" : ""}{Math.round(brief.preview.projected.cashDelta)}</span><span>Health {brief.preview.projected.healthDelta >= 0 ? "+" : ""}{Math.round(brief.preview.projected.healthDelta)}</span>{choice && <button className="phase4-action" onClick={() => void choose(choice)} disabled={status === "saving"}>Commit</button>}</div></div>; })}</div>}</article>
