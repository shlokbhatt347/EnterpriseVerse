...
export default function CompetitionPage() {
  ...
  useEffect(() => {
    if (!user || friendQuery.trim().length < 2) { setFriendResults([]); return; }
    const timer = window.setTimeout(() => { void searchPeople(friendQuery).then(setFriendResults).catch(() => setFriendResults([])); }, 180);
  ...
  const incomingRequests = useMemo(() => friends.filter((friend) => friend.status === "pending" && friend.addressee_id === user?.id), [friends, user?.id]);
  ...
  return <main className="competition-shell">
    ...
    {incomingRequests.length > 0 && <section className="friend-request-panel" aria-label="Friend requests"><div className="friend-request-heading"><div><span className="competition-kicker">NETWORK · ACTION REQUIRED</span><h2>Friend requests</h2><p>Requests remain visible until you accept or decline them.</p></div><span className="friend-request-count">{incomingRequests.length} pending</span></div><div className="friend-request-list">{incomingRequests.map((friend) => { const notification = incomingFriendNotifications.find((item) => String(item.metadata.friendship_id ?? "") === friend.id); return <article className="friend-request-card" key={friend.id}><div className="friend-request-avatar">F</div><div className="friend-request-copy"><strong>{notification?.title ?? "New friend request"}</strong><span>{notification?.body ?? "A founder wants to connect with you."}</span><small>Pending request</small></div><div className="friend-request-actions"><button className="competition-primary" disabled={busy} onClick={() => run(async () => { await respondFriendRequest(friend.id, "accepted"); if (notification) { await markNotificationRead(notification.id).catch(() => undefined); setNotifications((current) => current.map((item) => item.id === notification.id ? { ...item, read_at: new Date().toISOString() } : item)); } setFriends((current) => current.map((item) => item.id === friend.id ? { ...item, status: "accepted", updated_at: new Date().toISOString() } : item)); }, "Friend request accepted.")}>Accept</button><button className="competition-secondary" disabled={busy} onClick={() => run(async () => { await respondFriendRequest(friend.id, "declined"); if (notification) { await markNotificationRead(notification.id).catch(() => undefined); setNotifications((current) => current.map((item) => item.id === notification.id ? { ...item, read_at: new Date().toISOString() } : item)); } setFriends((current) => current.map((item) => item.id === friend.id ? { ...item, status: "declined", updated_at: new Date().toISOString() } : item)); }, "Friend request declined.")}>Decline</button></div></article>; })}</div></section>}
    ...
    <div className="competition-grid">
      <section className="competition-card room-card"><div className="card-top"><div><span className="competition-kicker">01 · PLAY</span><h2>{room ? "Your competition room" : "Start a competition"}</h2></div>{room && <span className={`room-status ${room.status}`}>{room.status}</span>}</div>
        {!room ? <div className="room-create"><label>Display name<input value={displayName} onChange={(e) => setDisplayName(e.target.value)} maxLength={80} /></label><button className="competition-primary full" disabled={busy || !displayName.trim()} onClick={() => run(async () => { const next = await createRoom(displayName); setRoom(next); setPlayers([{ room_id: next.id, user_id: user.id, display_name: displayName.trim(), ready: false, connected: true, joined_at: new Date().toISOString() }]); }, "Room created. Share the code with your friend.")}>Create friend room</button><div className="join-divider"><span>or join a room</span></div><div className="join-row"><input value={roomCode} onChange={(e) => setRoomCode(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 6))} placeholder="6-character code" maxLength={6} /><button className="competition-secondary" disabled={busy || roomCode.length !== 6} onClick={() => run(async () => { const found = await findRoomByCode(roomCode); if (!found) throw new Error("Room not found. Check the code and try again."); const player = await joinRoom(found, displayName); setRoom(found); setPlayers([{ room_id: found.id, user_id: player.user_id, display_name: player.display_name, ready: player.ready, connected: player.connected, joined_at: player.joined_at }]); await refreshRoom(found.id); }, "Joined the room.")}>Join</button></div></div> : <div><div className="room-code"><span>ROOM CODE</span><strong>{room.code}</strong><small>Share this code with your friend</small></div><div className="player-list">{players.map((player) => <div className="player-row" key={player.user_id}><span className={`presence ${player.connected ? "online" : ""}`} /><div><strong>{player.display_name}{player.user_id === room.host_id ? " · Host" : ""}</strong><small>{player.ready ? "Ready" : "Waiting"}</small></div><span className={player.ready ? "ready" : "waiting"}>{player.ready ? "READY" : "WAITING"}</span></div>)}</div>{room.status === "lobby" && <div className="room-actions"><button className="competition-secondary" disabled={busy} onClick={() => run(async () => { const next = await setReady(room.id, !Boolean(me?.ready)); setPlayers((current) => current.map((player) => player.user_id === user.id && next ? { ...player, ready: next.ready } : player)); })}>{me?.ready ? "I'm not ready" : "I'm ready"}</button>{room.host_id === user.id && <button className="competition-primary" disabled={busy || !everyoneReady} onClick={() => run(async () => { const next = await startRoom(room.id); setRoom(next); }, "Competition started!")}>Start competition</button>}</div>}{room.status === "active" && <div className="active-round"><span>ROUND {room.current_round} / {room.duration_rounds}</span><h3>Choose your strategy</h3><div className="decision-options">{[["balanced_growth","Balanced growth","Protect cash while building sustainable demand."],["aggressive_growth","Aggressive growth","Trade margin and cash for faster expansion."],["defensive_cash","Defensive cash","Prioritise liquidity and resilience."]].map(([id,title,desc]) => <button type="button" key={id} className={decision === id ? "selected" : ""} onClick={() => setDecision(id)}><strong>{title}</strong><span>{desc}</span></button>)}</div><button className="competition-primary full" disabled={busy} onClick={() => run(async () => { const result = await submitDecision(room.id, room.current_round, decision); if (result.current_round) setRoom((current) => current ? { ...current, current_round: result.current_round, status: result.completed ? "completed" : current.status } : current); }, "Decision submitted. Waiting for the other founders…")}>Submit round decision →</button></div>}{room.status === "completed" && <div className="completed"><strong>Competition complete.</strong><span>Your final result will appear on the leaderboard after scoring.</span></div>}</div>}
      </section>
