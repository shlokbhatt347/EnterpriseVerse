...
export default function CompetitionPage() {
  ...
  async function run(actionKey: ActionKey, fn: () => Promise<void>, success?: string) {
    ...
    try {
      ...
      setAction((current) => (current === actionKey ? null : current));
    }
  }

  const isBusy = (key: ActionKey) => action === key;
  ...
  return <main className="competition-shell">
    ...
    <div className="competition-grid">
      ...
      <section className="competition-card friends-card"><div className="card-top"><div><span className="competition-kicker">03 · NETWORK</span><h2>Friends</h2></div><span>{acceptedFriends.length} accepted</span></div><div className="friend-add"><input value={friendQuery} onChange={(e) => setFriendQuery(e.target.value)} placeholder="Search name or exact email" aria-label="Search friends" /><span className="friend-hint">Type at least 2 characters</span></div>{friendResults.length>0 && <div className="friend-search-results">{friendResults.map((person) => { const addKey = `friend:${person.user_id}`; return <div className="friend-search-row" key={person.user_id}><div><strong>{person.display_name}</strong><small>{person.email ?? "EnterpriseVerse founder"}</small></div><button className="competition-secondary" disabled={action !== null && action !== addKey} onClick={() => run(addKey, async () => { const friendshipId = await sendFriendRequest(person.user_id); setFriendQuery(""); setFriendResults([]); setFriends((current) => [{ id: friendshipId, requester_id: user.id, addressee_id: person.user_id, status: "pending", created_at: new Date().toISOString(), updated_at: new Date().toISOString() }, ...current]); }, "Friend request sent instantly. The recipient will see it in their requests.")}>{isBusy(addKey) ? "Sending…" : "Add friend"}</button></div>; })}</div>}{acceptedFriends.length ? <div className="friend-list">{acceptedFriends.map((friend) => <div className="friend-row" key={friend.id}><div><strong>Connected founder</strong><small>Friends since {new Date(friend.updated_at).toLocaleDateString()}</small></div><span className="status-accepted">FRIENDS</span></div>)}</div> : <div className="empty-state">Search for a founder by display name or exact email to add them.</div>}</section>
    </div>
