...
type LeaderboardRow = { user_id: string; scope: string; score: number; rank?: number | null; metrics: Record<string, unknown>; achieved_at: string; display_name?: string | null };
...
export async function joinRoom(room: CompetitionRoom, displayName: string) {
  ...
  if (!normalized) throw new Error("Display name is required.");
  if (normalized.length > 80) throw new Error("Display name is too long.");
  const raw = await rest<CompetitionPlayer | CompetitionPlayer[]>("/rest/v1/rpc/join_competition_room", { method: "POST", body: JSON.stringify({ p_room_id: room.id, p_display_name: normalized }) });
  const player = Array.isArray(raw) ? raw[0] : raw;
  if (!player?.user_id) throw new Error("The server did not confirm your room membership.");
  return player;
}
