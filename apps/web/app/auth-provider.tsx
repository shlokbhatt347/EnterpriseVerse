...
function cachedAccountUser(): AccountUser | null {
  ...
  return cached ? { id: cached.id, displayName: cached.displayName, email: cached.email, emailConfirmed: cached.emailConfirmed } : null;
}

function queueCloudSave(key: string, value: unknown) {
  const previous = pendingSaves.get(key) ?? Promise.resolve();
  const next = previous.catch(() => undefined).then(() => saveCloudSave(key, value)).finally(() => { if (pendingSaves.get(key) === next) pendingSaves.delete(key); });
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    ...
    } catch { setUser(guestUser()); setMode("guest"); setCloudReady(false); }
    setAuthReady(true);
  }, []);
  ...
  const signInWithEmail = useCallback(async (email: string, password: string) => {
    try {
      ...
      setUser(next); setMode("email"); setCloudReady(false); return { ok: true };
    } catch (error) { return { ok: false, error: error instanceof Error ? error.message : "Unable to sign in." }; }
  }, []);

  const signUpWithEmail = useCallback(async (email: string, password: string, displayName: string) => {
    try {
      const next = await supabaseSignUp(email.trim().toLowerCase(), password, displayName.trim());
      setUser(next);
      setMode("email");
      setCloudReady(false);
      return { ok: true };
  ...
  const signOut = useCallback(async () => {
    ...
    try { await supabaseSignOut(); } finally { setCloudReady(false); setUser(guestUser()); setMode("guest"); }
  }, [mode, user?.id]);
  ...
  const deleteBusiness = useCallback(async (key: string) => {
    ...
    if (mode === "email" && cloudReady) { pendingSaves.delete(key); try { await deleteCloudSave(key); } catch { /* local state already deleted */ } }
  }, [cloudReady, mode]);
