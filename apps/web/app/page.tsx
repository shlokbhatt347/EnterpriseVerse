...
type View = "overview" | "operations" | "market" | "people" | "finance" | "learning";
const SAVE_KEY = "enterpriseverse:active-business:v1";

const money = (value: number) => `₹${Math.round(value).toLocaleString("en-IN")}`;
const clamp = (value: number) => Math.max(0, Math.min(100, Math.round(value)));
const titleCase = (value: string) => value.replaceAll("-", " ").replaceAll("_", " ").replace(/\b\w/g, (letter) => letter.toUpperCase());
...
export default function Home() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        const saved = JSON.parse(raw) as SimulationState;
        if (saved?.business?.name && saved?.business?.day && Array.isArray(saved.events) && Array.isArray(saved.log)) {
          setState(saved);
          setStarted(true);
          setRestored(true);
        }
  ...
  useEffect(() => {
    ...
    try {
      ...
      setSaveNotice("Saved");
      const timer = window.setTimeout(() => setSaveNotice(""), 1200);
  ...
  const resetSimulation = () => {
    if (!window.confirm("Start a new business? Your saved simulation will be removed.")) return;
    window.localStorage.removeItem(SAVE_KEY);
    setState(null);
    setStarted(false);
    setRestored(false);
  ...
  const learning = assessLearning(state);
  ...
  return <main className="shell">
    ...
    <div className="app-layout">
      <aside className="sidebar"><div className="side-title">COMMAND</div>{nav("overview", "Overview")}{nav("operations", "Operations")}{nav("market", "Market")}{nav("people", "People")}{nav("finance", "Finance")}{nav("learning", "Learning")}<div className="side-divider" /><div className="side-title">BUSINESS</div><div className="side-stat"><span>Reputation</span><b>{state.business.reputation}/100</b></div><div className="side-stat"><span>Market share</span><b>{state.business.marketShare.toFixed(1)}%</b></div><div className="side-stat"><span>Inventory</span><b>{state.business.inventory}</b></div><button type="button" className="reset-button" onClick={resetSimulation}>New business</button></aside>
      <div className="content dashboard">
        <section className="hero"><div><div className="eyebrow">{state.business.industry} · Executive dashboard</div><h1>{state.business.name}</h1><div className="muted">{state.business.idea}</div></div><div className="hero-actions"><span className={`pill ${learning.level}`}>{learning.level} · {learning.overallScore}/100</span><button type="button" className="primary" onClick={nextDay} disabled={Boolean(event)}>End Day →</button></div></section>
        ...
        {(view === "overview" || view === "operations") && <section className="operations card"><div className="section-head"><div><div className="eyebrow">CEO control room</div><div className="event-title">Operate the enterprise</div><div className="muted">Decisions are persistent. Price, quality, marketing, capacity and finance all feed the simulation.</div></div><span className="pill">Debt · {money(kpis.debt)}</span></div>{actionError && <div className="error">{actionError}</div>}<div className="action-grid"><div className="action-card"><strong>Pricing</strong><span>Current {money(kpis.price)}</span><div className="action-row"><input value={priceInput} onChange={(e) => setPriceInput(e.target.value)} type="number" min="20" max="2000" /><button type="button" className="secondary" onClick={() => runAction({ type: "set_price", price: Number(priceInput) })}>Set</button></div></div><div className="action-card"><strong>Marketing</strong><span>Build brand awareness</span><div className="action-row"><input value={budgetInput} onChange={(e) => setBudgetInput(e.target.value)} type="number" min="0" /><button type="button" className="secondary" onClick={() => runAction({ type: "marketing", budget: Number(budgetInput) })}>Launch</button></div></div><div className="action-card"><strong>Quality</strong><span>Current {kpis.quality}/100</span><div className="action-row"><input value={qualityInput} onChange={(e) => setQualityInput(e.target.value)} type="number" min="0" /><button type="button" className="secondary" onClick={() => runAction({ type: "improve_quality", investment: Number(qualityInput) })}>Invest</button></div></div><div className="action-card"><strong>Inventory</strong><span>{state.business.inventory} units available</span><div className="action-row"><input value={stockInput} onChange={(e) => setStockInput(e.target.value)} type="number" min="1" /><button type="button" className="secondary" onClick={() => runAction({ type: "restock", units: Number(stockInput) })}>Restock</button></div></div><div className="action-card"><strong>Hiring</strong><span>{kpis.employees} employees · capacity {kpis.productionCapacity}</span><div className="action-row"><input value={hireInput} onChange={(e) => setHireInput(e.target.value)} type="number" min="1" max="20" /><button type="button" className="secondary" onClick={() => runAction({ type: "hire", employees: Number(hireInput) })}>Hire</button></div></div><div className="action-card"><strong>Finance</strong><span>Debt {money(kpis.debt)}</span><div className="action-row"><input value={loanInput} onChange={(e) => setLoanInput(e.target.value)} type="number" min="1000" /><button type="button" className="secondary" onClick={() => runAction({ type: "loan", amount: Number(loanInput) })}>Borrow</button><button type="button" className="secondary" onClick={() => runAction({ type: "repay_loan", amount: Number(loanInput) })}>Repay</button></div></div></div><div className="kpi-strip"><span>Quality <b>{kpis.quality}</b></span><span>Awareness <b>{kpis.brandAwareness}</b></span><span>Satisfaction <b>{kpis.customerSatisfaction}</b></span><span>Employees <b>{kpis.employees}</b></span><span>Capacity <b>{kpis.productionCapacity}</b></span></div></section>}
