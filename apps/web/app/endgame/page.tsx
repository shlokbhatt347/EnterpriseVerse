...
export default function EndgamePage() {
  ...
  const simulationState = state as SimulationState | null;
  const snapshot = simulationState ? getPhase7CompanySnapshot(simulationState) : null;
  const score = simulationState && snapshot ? calculatePhase7Score(snapshot, simulationState) : null;
  const achievements = simulationState && snapshot && score ? getPhase7Achievements(snapshot, score, simulationState) : [];
  const topRankings = rankings.slice(0, 8);

  if (!simulationState || !snapshot || !score) {
    return (
      <main className="endgame-shell">
        <section className="empty-legacy panel">
          <div className="empty-icon">◆</div>
          <h2>{status === "error" ? "Your enterprise could not be loaded." : "Your enterprise legacy starts here."}</h2>
          <p>{error ?? "Create or resume a company to unlock your score, achievements and endgame analytics."}</p>
          <div className="empty-actions"><Link href="/founder" className="primary-button">Build your business →</Link><Link href="/day1" className="secondary-button">Open enterprise</Link></div>
        </section>
      </main>
    );
  }
  ...
  return (
    <CanonicalShell state={simulationState} syncStatus={status}>
      <main className="endgame-shell">
