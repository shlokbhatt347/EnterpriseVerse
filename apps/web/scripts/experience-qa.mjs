...
const shell = readFileSync(join(app, "experience", "CanonicalShell.tsx"), "utf8");
const shellCss = readFileSync(join(app, "experience", "canonical-shell.css"), "utf8");
...
for (const relativePath of canonicalPages) {
  ...
  check(
    source.includes("CanonicalShell"),
    `${relativePath} must render through CanonicalShell.`
  );
}
