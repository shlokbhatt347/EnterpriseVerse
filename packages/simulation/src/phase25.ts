...
export interface FounderDashboard { cash: number; runwayDays: number; revenue: number; profit: number; customers: number; marketShare: number; businessHealth: number; milestones: FirstSessionMilestone[]; }
...
const direction = (value: number): "positive" | "negative" | "neutral" => value > 0.5 ? "positive" : value < -0.5 ? "negative" : "neutral";
