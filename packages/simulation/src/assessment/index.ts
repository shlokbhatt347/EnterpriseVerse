...
export function assessRun(business: Business, outcomes: DecisionOutcome[] = []): RunAssessment {
  ...
  const growth = clamp(business.marketShare * 3 + customers.length);
  const decisionQuality = safeOutcomes.length ? safeOutcomes.reduce((sum, o) => sum + o.score, 0) / safeOutcomes.length : 50;
  ...
  const blindSpots = [business.expenses > business.revenue ? "Cost control" : "Scaling discipline", business.reputation < 50 ? "Reputation management" : "Defensive strategy", customers.length < 8 ? "Customer acquisition" : "Customer retention"];
  const lessons = safeOutcomes.filter((o) => o.score < 45).slice(-3).map((o) => o.explanation);
