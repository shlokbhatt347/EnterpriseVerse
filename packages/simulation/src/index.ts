...
function generateEvent(day: number, business: Business): SimulationEvent {
  ...
  if (cycle === 1) {
    return {
      ...
      message: "Your main supplier says input costs will rise by 12%. Decide how to protect your margin.",
      choices: [
  ...
  if (cycle === 2) {
    return {
      ...
      message: "A local trend is driving extra demand. You can invest in stock or protect your cash.",
      choices: [
        choice("stock", "Buy extra inventory", { cash: -1_200, revenue: 2_400, reputation: 1, inventory: 30, customers: 5 }),
        choice("normal", "Keep operations unchanged", { revenue: 800, customers: 2, inventory: -8 }),
        choice("premium", "Raise prices and test demand", { revenue: 1_400, reputation: -1, customers: 1, inventory: -6 }),
      ],
    };
  }
  if (cycle === 3) {
    return {
      ...
      message: "A visible customer says their order was late. Your response will affect trust and reputation.",
      choices: [
  ...
  if (cycle === 4) {
    return {
      ...
      message: "A nearby competitor has launched a discount campaign. You must decide whether to react.",
      choices: [
        choice("match", "Match their discount", { cash: -500, revenue: 1_500, reputation: 1, customers: 3 }),
        choice("differentiate", "Differentiate on quality", { revenue: 1_100, reputation: 3, customers: 2 }),
  ...
  return {
    ...
    message: `${business.name} has attracted attention. An investor wants a short meeting. Decide whether to pursue growth capital.`,
    choices: [
      choice("pitch", "Prepare a growth pitch", { cash: 10_000, reputation: 3, marketShare: 2 }),
...
export function createBusiness(input: {
  ...
  const founders = input.founderNames.map((name, index) => ({
    id: `founder-${index + 1}`,
    name,
    cash: Math.round(startingCash / Math.max(1, input.founderNames.length)),
    reputation: 50,
  }));
  ...
  return {
    ...
    log: [`Day 1: ${business.name} opened for business with ₹${startingCash.toLocaleString("en-IN")} starting capital.`],
  };
}
...
export function advanceDay(state: SimulationState): SimulationState {
  ...
  const nextCustomers = business.customers.map((customer, index) =>
    index < Math.min(unitsSold, business.customers.length)
  ...
  return {
    ...
    log: [
      ...state.log,
      `Day ${nextBusiness.day}: sold ${unitsSold} units for ₹${revenue.toLocaleString("en-IN")}; operating costs were ₹${fixedExpense.toLocaleString("en-IN")}.`,
    ],
  };
}
...
export function applyChoice(state: SimulationState, selected: SimulationChoice): SimulationState {
  ...
  const customers = effects.customerTrust
    ? business.customers.map((customer, index) =>
        index === 0 ? { ...customer, trust: clamp(customer.trust + effects.customerTrust) } : customer,
      )
    : business.customers;
  const suppliers = effects.supplierRelationship
    ? business.suppliers.map((supplier, index) =>
        index === 0 ? { ...supplier, relationship: clamp(supplier.relationship + effects.supplierRelationship) } : supplier,
      )
    : business.suppliers;
