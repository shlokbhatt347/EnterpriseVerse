...
declare
  ...
  normalized_decision text := lower(trim(coalesce(p_decision_id, '')));
begin
  if auth.uid() is null then raise exception 'Authentication required'; end if;
  if p_round < 1 or normalized_decision not in ('aggressive_growth', 'balanced_growth', 'defensive_cash') then
    raise exception 'Invalid competition decision';
  end if;
  ...
  values (p_room_id, auth.uid(), p_round, normalized_decision)
