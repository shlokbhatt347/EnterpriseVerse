...
alter table public.businesses
  add column if not exists team_size text not null default 'solo';

alter table public.businesses
  drop constraint if exists businesses_team_size_check;
alter table public.businesses
  add constraint businesses_team_size_check
  check (team_size in ('solo','pair','trio','company'));

create index if not exists business_members_user_idx
  on public.business_members(user_id, joined_at desc);
create index if not exists business_invitations_invitee_idx
  on public.business_invitations(invitee_id, status, created_at desc);
...
-- Private collaboration visibility.
...
create policy business_invitations_select_participant
...
create policy business_invitations_insert_inviter
...
create policy business_invitations_update_participant
...
grant select, insert on public.business_members to authenticated;
...
-- Safe friend/user search. The input is text and is never cast to UUID.
-- Email is only returned for an exact normalized email match; display-name search
...
as $$
  ...
  where (select auth.uid()) is not null
    ...
    and (
      lower(p.display_name) like '%' || q.value || '%'
      or lower(coalesce(u.email, '')) = q.value
    )
    and p.user_id <> (select auth.uid())
  order by case when lower(coalesce(u.email, '')) = q.value then 0 else 1 end,
...
-- Authoritative enterprise-size rules. These are enforced in the database,
-- independent of frontend controls.
create or replace function public.business_participant_limit(p_team_size text)
returns integer
language sql
immutable
as $$
  select case p_team_size when 'solo' then 1 when 'pair' then 2 when 'trio' then 3 else null end;
$$;
...
begin
  ...
  if exists (select 1 from public.business_members where business_id = p_business_id and user_id = p_invitee_id) then
    raise exception 'That user is already a participant';
  end if;
  if exists (select 1 from public.business_invitations where business_id = p_business_id and invitee_id = p_invitee_id and status = 'pending') then
    raise exception 'An invitation is already pending for that user';
  end if;

  insert into public.business_invitations(business_id, inviter_id, invitee_id)
  values (p_business_id, actor, p_invitee_id)
...
begin
  ...
  values (invite.business_id, actor, 'founder')
...
begin
  ...
  initial_limit := public.business_participant_limit(p_team_size);
...
-- Prevent direct client-side mutation of the authoritative membership/invite paths.
...
-- Keep businesses readable only by their owner; membership is handled by the
-- dedicated collaboration functions to avoid policy recursion and limit bypasses.
