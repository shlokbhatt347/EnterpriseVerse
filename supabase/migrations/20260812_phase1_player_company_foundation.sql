...
alter table public.profiles add constraint profiles_active_role_check
  check (active_role is null or active_role in ('ceo','cfo','cmo','coo','cto','chro'));
...
alter table public.business_invitations add column if not exists requested_role text not null default 'founder';
...
begin
  ...
  insert into public.business_invitations(business_id,inviter_id,invitee_id,requested_role) values(p_business_id,actor,p_invitee_id,p_requested_role) returning id into invitation_id;
